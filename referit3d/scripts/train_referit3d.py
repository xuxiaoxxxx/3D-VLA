...
def log_train_test_information():
        ...
        for phase in ['train', 'test']:
            ...
            if args.obj_cls_alpha == 0:
                ...
                info += ', Object-Clf-2d-Acc: {:.4f}'.format(meters[phase + '_object_cls_2d_acc'])

            if args.lang_cls_alpha > 0:
                info += ', Text-Clf-Acc: {:.4f}'.format(meters[phase + '_txt_cls_acc'])
            info += ', Object-Clf-loss: {:.4f}'.format(meters[phase + '_obj_loss'])
            info += ', Text-Clf-loss: {:.4f}'.format(meters[phase + '_text_loss'])
            # info += ', Listening-loss: {:.4f}'.format(meters[phase + '_refer_loss'])
            # if args.contrastive_alpha:
            info += ', Contrastive-loss: {:.4f}'.format(meters[phase + '_contrastive_loss'])
...
if __name__ == '__main__':
    ...
    param_list=[
        ...
        {'params':model.image_adapter.parameters(), 'lr': args.init_lr},
        {'params':model.pc_adapter.parameters(), 'lr': args.init_lr},
        {'params':model.text_adapter.parameters(), 'lr': args.init_lr},
    ...
    lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer,[20, 25, 30, 40, 50], gamma=0.65)
