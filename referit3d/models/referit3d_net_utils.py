...
def make_batch_keys(args, extras=None):
    ...
    batch_keys = ['objects', 'lang_feat', 'images', 'target_pos']  # all models use these
...
def single_epoch_train(model, data_loader, criteria, optimizer, device, pad_idx, args, tokenizer=None,epoch=None):
    ...
    obj_3d_loss_mtr = AverageMeter()
    text_loss_mtr = AverageMeter()
    contrastive_loss_mtr = AverageMeter()
    ref_acc_mtr = AverageMeter()
    cls_2d_acc_mtr = AverageMeter()
    cls_3d_acc_mtr = AverageMeter()
    ...
    for batch in tqdm.tqdm(data_loader):
        ...
        # Forward pass
        LOSS, LOGITS, LOGITS_after, CLASS_2D_LOGITS, CLASS_3D_LOGITS, LANG_LOGITS, obj_clf_loss, lang_clf_loss, contrastive_loss = model(batch, epoch)
        LOSS = LOSS.mean()
        
        obj_clf_loss = obj_clf_loss.mean()
        # refer_loss = refer_loss.mean()
        lang_clf_loss = lang_clf_loss.mean()
        # print("LOSS:", LOSS)
        res = {}
        res['logits'] = LOGITS
        res['logits_after'] = LOGITS_after
        res['class_logits'] = CLASS_3D_LOGITS
        res['class_2d_logits'] = CLASS_2D_LOGITS
        ...
        # referential_loss_mtr.update(refer_loss.item(), batch_size)
        obj_loss_mtr.update(obj_clf_loss.item(), batch_size)
        text_loss_mtr.update(lang_clf_loss.item(), batch_size)

        # if args.contrastive_alpha:
        contrastive_loss = contrastive_loss.mean()
        contrastive_loss_mtr.update(contrastive_loss.item(), batch_size)
        ...
        if args.obj_cls_alpha == 0:
            ...
            cls_3d_acc_mtr.update(cls_b_acc, batch_size)

            cls_b_acc, _ = cls_pred_stats(res['class_2d_logits'], batch['class_labels'], ignore_label=pad_idx)
            cls_2d_acc_mtr.update(cls_b_acc, batch_size)

        # print(args.lang_cls_alpha > 0)
        if args.lang_cls_alpha == 0:
        ...
        #     print("txt_acc_mtr.avg")
        # print(a)
    metrics['train_total_loss'] = total_loss_mtr.avg
    metrics['train_referential_acc'] = ref_acc_mtr.avg
    metrics['train_object_cls_acc'] = cls_3d_acc_mtr.avg
    metrics['train_object_cls_2d_acc'] = cls_2d_acc_mtr.avg
    metrics['train_txt_cls_acc'] = txt_acc_mtr.avg
    # metrics['train_refer_loss'] = referential_loss_mtr.avg
    metrics['train_obj_loss'] = obj_loss_mtr.avg
    metrics['train_text_loss'] = text_loss_mtr.avg
    # if args.contrastive_alpha:
    metrics['train_contrastive_loss'] = contrastive_loss_mtr.avg
    return metrics
...
def evaluate_on_dataset(model, data_loader, criteria, device, pad_idx, args, randomize=False, tokenizer=None):
    ...
    metrics['test_object_cls_acc'] = cls_3d_acc_mtr.avg
    metrics['test_object_cls_2d_acc'] = cls_2d_acc_mtr.avg
    metrics['test_txt_cls_acc'] = txt_acc_mtr.avg
    # metrics['test_refer_loss'] = referential_loss_mtr.avg
    metrics['test_obj_loss'] = obj_loss_mtr.avg
    metrics['test_text_loss'] = text_loss_mtr.avg
    # if args.contrastive_alpha:
    metrics['test_contrastive_loss'] = contrastive_loss_mtr.avg
    return metrics
