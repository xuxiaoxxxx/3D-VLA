...
class ListeningDataset(Dataset):
    ...
    def get_reference_data(self, index):
        ...
        return scan, target, tokens, is_nr3d, scan_id, ref['tokens']
    ...
    def pad_image(self, images, max_context_size):
        n_pad = max_context_size - len(images)

        if n_pad > 0:
            shape = (max_context_size, images.shape[1])
            temp = np.zeros(shape, dtype=images.dtype)
    ...
    def __getitem__(self, index):
        res = dict()
        scan, target, tokens, is_nr3d, scan_id, lang_feat = self.get_reference_data(index)
        ...
        target_class_mask = np.zeros(self.max_context_size, dtype=np.bool_)                     # lxx：我这改成np.bool_原来是np.bool
        ...
        # print("tokens:", lang_feat)
        choice_idx = np.random.choice(lang_len, int(float(self.word_erase) * lang_len))
        # breakpoint()
        for i in choice_idx:
            lang_feat[i] = ""
        ...
        lang_mask = np.zeros((self.max_des_len, ))
        ...
        res['text'] = res['utterance']

        res['lang_feat'] = lang_feat
        res['lang_mask'] = lang_mask
        ...
        # load the 2D pic nr3d
        path_2d_base = os.path.join('/data/xuxiaoxu/test/scannet_img_clip_all', scan.scan_id)

        # # load the 2D pic sr3d
        # path_2d_base = os.path.join('/data/xuxiaoxu/test/scannet_img_sr3d_clip_all', scan.scan_id)

        
        images = []
        List = []
        for o in context:
            image_path = os.path.join(path_2d_base, str(o.object_id) + '.npy')
            img_feature = np.load(image_path)[0]
            images.append(img_feature)
        images = np.array(images)
        res['images'] = self.pad_image(images, self.max_context_size)
        return res
