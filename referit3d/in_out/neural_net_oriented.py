...
g_label_names = ['unannotated', 'chair', 'table', 'desk', 'bed', 'bookshelf', 'sofa', 'sink', 'bathtub', 'toilet', 'curtain', 'counter', 'door', 'window', 'shower curtain', 'refridgerator', 'picture', 'cabinet', 'otherfurniture']

def get_raw2scannetv2_label_map():
    lines = [line.rstrip() for line in open('/data/xuxiaoxu/dataset/scannet/scannetv2-labels.combined.tsv')]
...
#     for el in g_label_names:
#         raw_to_idx[el] = i
#         i += 1

#     for k, v in g_raw2scannetv2.items():
#         class_to_idx[k] = raw_to_idx[v]


#     for scan in all_scans:
#         idx = np.array([o.object_id for o in scan.three_d_objects])
#         # instance_labels.update([o.instance_label for o in scan.three_d_objects])
#         instance_labels += [o.instance_label for o in scan.three_d_objects]
#         assert np.all(idx == np.arange(len(idx)))  # assert the list of objects-ids -is- the range(n_objects).
#                                                    # because we use this ordering when we sample objects from a scan.
#     all_scans = {scan.scan_id: scan for scan in all_scans}  # place scans in dictionary
#     print(len(instance_labels))
#     print(instance_labels[0])
#     weight = [0] * 19
#     for el in instance_labels:
#         name = class_to_idx[el]
#         weight[name] += 1
#     print(weight)
#     print(a)
